    ...
    <form action="{{ route('softwares.update', $software->software_id) }}" method="POST">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="f_name" id="f_name" class="form-control"
                    value="{{ old('f_name', $software->f_name) }}" placeholder="ชื่อ" required>
            </div>
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="l_name" id="l_name" class="form-control"
                    value="{{ old('l_name', $software->l_name) }}" placeholder="นามสกุล" required>
            </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="software_name" id="software_name" class="form-control"
                placeholder="ชื่อระบบ" value="{{ old('software_name', $software->software_name) }}" required>
        </div>

        <!-- Problem -->
        <div class="mb-3">
            <label for="problem" class="form-label">ปัญหาที่เกิดขึ้นจากระบบเดิม *</label>
            <textarea name="problem" id="problem" rows="3" class="form-control" placeholder="ปัญหาที่เกิดขึ้นจากระบบเดิม" required>{{ old('problem', $software->problem) }}</textarea>
        </div>

        <!-- Purpose -->
        <div class="mb-3">
            <label for="purpose" class="form-label">วัตถุประสงค์การพัฒนาระบบใหม่ *</label>
            <textarea name="purpose" id="purpose" rows="3" class="form-control" placeholder="วัตถุประสงค์การพัฒนาระบบใหม่" required>{{ old('purpose', $software->purpose) }}</textarea>
        </div>

        <!-- Target -->
        <div class="mb-3">
            <label for="target" class="form-label">กลุ่มเป้าหมายการใช้งาน *</label>
            <textarea name="target" id="target" rows="2" class="form-control" placeholder="กลุ่มเป้าหมายการใช้งาน" required>{{ old('target', $software->target) }}</textarea>
        </div>
